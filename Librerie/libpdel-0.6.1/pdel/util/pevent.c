...
#define READABLE_EVENTS		(PD_POLLIN | PD_POLLRDNORM | PD_POLLERR \
				    | PD_POLLHUP | PD_POLLNVAL)
#define WRITABLE_EVENTS		(PD_POLLOUT | PD_POLLWRNORM | PD_POLLWRBAND \
				    | PD_POLLERR | PD_POLLHUP | PD_POLLNVAL)
...
struct pevent_ctx {
	...
	pd_pollfd		*fds;		/* poll(2) fds array */
...
pevent_ctx_create(const char *mtype, const pthread_attr_t *attr)
{
	...
	if (attr != NULL) {
		...
		if ((errno = pthread_attr_init(&ctx->attr)) != 0)
			goto fail;
		ctx->has_attr = 1;
		/* Darn pthread-win32 isn't properly constified */
...
#ifdef WIN32
	if (pd_socketpair(AF_INET, SOCK_STREAM, 0, ctx->pipe) == -1)
#else
	if (pipe(ctx->pipe) == -1)
#endif
		goto fail;
	...
	enum pevent_type type, ...)
{
	...
	DBG(PEVENT, "new ev %p in ctx %p", ev _ ctx);
...
pevent_unregister(struct pevent **peventp)
{
	...
	if (ev == NULL)
		return;
	ctx = ev->ctx;
	DBG(PEVENT, "unregister ev %p in ctx %p", ev _ ctx);
	...
	/* Lock context */
	MUTEX_LOCK(&ctx->mutex, ctx->mutex_count);
	DBG(PEVENT, "unregister ev %p in ctx %p: lock", ev _ ctx);
	...
	/* Unlock context */
	DBG(PEVENT, "unregister ev %p in ctx %p: unlock", ev _ ctx);
	MUTEX_UNLOCK(&ctx->mutex, ctx->mutex_count);
}
...
pevent_trigger(struct pevent *ev)
{
	...
	/* Get context */
	ctx = ev->ctx;
	DBG(PEVENT, "trigger ev %p in ctx %p", ev _ ctx);
	assert(ev->magic == PEVENT_MAGIC);
...
loop:
	...
	TAILQ_FOREACH(ev, &ctx->events, next) {
		...
		case PEVENT_TIME:
			...
			if (pd_timercmp(&ev->when, &now, <=))
				millis = 0;
			else {
				pd_timersub(&ev->when, &now, &remain);
	...
	r = pd_poll(ctx->fds, poll_idx, timeout);
	...
	/*
	 * Check for dangling bytes on the wakeup pipe, seems to 
	 * happen on Win32.
	 */
	fd = &ctx->fds[0];
	if ((fd->revents & READABLE_EVENTS) != 0) {
		ctx->notified = 1;
	}
	...
	while (1) {
		...
		if (ev == NULL || (ev->flags & PEVENT_OCCURRED) == 0)
			break;
		DBG(PEVENT, "ctx %p thread servicing ev %p", ctx _ ev);
...
pevent_ctx_service(struct pevent *ev)
{
	...
	if ((ev->flags & PEVENT_OWN_THREAD) != 0) {
		...
		pthread_detach(tid);
		DBG(PEVENT, "dispatched thread %p for ev %p", tid _ ev);
		return;
	}
...
try_again:
	...
	DBG(PEVENT, "locking ctx %p for ev %p", ctx _ ev);
	MUTEX_LOCK(&ctx->mutex, ctx->mutex_count);
...
pevent_ctx_notify(struct pevent_ctx *ctx)
{
	...
	if (!ctx->notified) {
		(void)pd_write(ctx->pipe[1], &pevent_byte, 1);
		ctx->notified = 1;
	}
}
...
_pevent_unref(struct pevent *ev)
{
	...
	DBG(PEVENT, "ev %p refs %d -> %d", ev _ ev->refs _ ev->refs - 1);
	assert(ev->refs > 0);
	assert(ev->magic == PEVENT_MAGIC);
	MUTEX_LOCK(&ctx->mutex, ctx->mutex_count);
	if (--ev->refs > 0) {
...
{
	DBG(PEVENT, "ctx %p refs %d -> %d", ctx _ ctx->refs _ ctx->refs - 1);
	assert(ctx->refs > 0);
	assert(ctx->magic == PEVENT_CTX_MAGIC);
	if (--ctx->refs > 0) {
		MUTEX_UNLOCK(&ctx->mutex, ctx->mutex_count);
	...
	assert(pd_pthread_isnull(ctx->thread));
	(void)pd_close(ctx->pipe[0]);
	(void)pd_close(ctx->pipe[1]);
	MUTEX_UNLOCK(&ctx->mutex, ctx->mutex_count);
	pthread_mutex_destroy(&ctx->mutex);
	if (ctx->has_attr)
		pthread_attr_destroy(&ctx->attr);
