	...
	for (s = nbuf; s != NULL; s = strchr(s + 1, STRUCTS_SEPARATOR)) {
		...
		*s = '\0';
		if ((atype = structs_find(type,
		    nbuf + (s != nbuf), (const void **)&ary, 1)) == NULL) {
 ...
 */
int
structs_array_setsize(const struct structs_type *type,
		      const char *name, u_int nitems, void *data, int do_init)
{
	struct structs_array *ary = data;
	const struct structs_type *etype;
	const char *mem_type;
	...
	/* Reallocate array, leaving room for new element and a shift */
	if ((mem = REALLOC(mem_type,
	    ary->elems, nitems * etype->size)) == NULL)
		return (-1);
